// ===== reply-library.js 修复版 =====
// 修复：面板切换残留问题，强制显示/隐藏对应区域
...
function _showExportUI() {
    ...
    const groupDescMap = { replies: '仅导出指定分组的字卡内容', pokes: '仅导出指定分组的拍一拍内容', statuses: '仅导出指定分组的对方状态内容', announcement: '选择要导出的公告内容模块', };
    if (anyGroupExists) {
...
panel.querySelector('#_gep_cancel').onclick = () => overlay.remove();
    ...
    panel.querySelector('#_gep_confirm').onclick = () => {
        ...
        const libraryData = { exportDate: new Date().toISOString(), modules: cfg.moduleTag, [cfg.itemKey]: [...allItems], [cfg.groupKey]: exportGroups, _groupExport: true, _groupExportType: type };
