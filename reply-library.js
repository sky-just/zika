...
// 注册“组字卡”子标签（在侧边栏或主标签中）
function initComboCardTab() {
    // 在侧边栏添加“组字卡”按钮
    const sidebar = document.querySelector('.modal-sidebar');
    if (sidebar && !document.querySelector('.sidebar-btn[data-major="combos"]')) {
        const btn = document.createElement('button');
        btn.className = 'sidebar-btn';
        btn.dataset.major = 'combos';
        btn.innerHTML = '<i class="fas fa-puzzle-piece"></i><span>组字卡</span>';
        btn.addEventListener('click', switchToCombosTab);
        sidebar.appendChild(btn);
    }
}

let currentComboTab = false;

function switchToCombosTab() {
    currentMajorTab = 'combos';
    document.querySelectorAll('.sidebar-btn').forEach(b => b.classList.remove('active'));
    document.querySelector('.sidebar-btn[data-major="combos"]').classList.add('active');
...
function renderComboCardList() {
    ...
    toolbar.style.cssText = 'padding:10px 15px;border-bottom:1px solid var(--border-color);display:flex;gap:8px;align-items:center;';
    toolbar.innerHTML = `
        <button id="add-combo-btn" class="modal-btn modal-btn-primary" style="flex:1;">
            <i class="fas fa-plus"></i> 创建组字卡
        </button>
    `;
    list.appendChild(toolbar);

    if (!window.comboCards || window.comboCards.length === 0) {
        list.innerHTML += `<div class="env-empty" style="padding:40px;">
            <i class="fas fa-puzzle-piece" style="font-size:32px;opacity:0.3;"></i>
            <p>还没有组字卡</p>
            <span>点击上方按钮创建，一条回复可包含多条字卡</span>
        </div>`;
        return;
    }
    ...
    window.comboCards.forEach((combo, index) => {
        const card = document.createElement('div');
        card.className = 'custom-reply-item';
        card.style.cssText = 'justify-content:space-between;margin-bottom:10px;';
        card.innerHTML = `
            <div style="flex:1;">
                <div style="font-weight:600;color:var(--text-primary);">${combo.name}</div>
                <div style="font-size:11px;color:var(--text-secondary);margin-top:2px;">
                    ${combo.items.length} 条字卡 · 分隔符「${combo.separator || ''}」
                </div>
                <div style="font-size:11px;color:var(--text-secondary);margin-top:4px;font-style:italic;">
                    预览：${combo.items.slice(0,3).join(combo.separator || ' ')}${combo.items.length > 3 ? '…' : ''}
                </div>
            </div>
            <div style="display:flex;gap:6px;">
                <button class="reply-action-mini edit-btn" data-index="${index}" title="编辑"><i class="fas fa-edit"></i></button>
                <button class="reply-action-mini delete-btn" data-index="${index}" title="删除"><i class="fas fa-trash"></i></button>
            </div>
        `;
    ...
    // 绑定事件
    document.getElementById('add-combo-btn').addEventListener('click', () => openComboEditor(-1));
    list.querySelectorAll('.edit-btn').forEach(btn => {
        btn.addEventListener('click', () => openComboEditor(parseInt(btn.dataset.index)));
    });
    list.querySelectorAll('.delete-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            const idx = parseInt(btn.dataset.index);
            if (confirm(`确定删除组字卡「${window.comboCards[idx].name}」吗？`)) {
                window.comboCards.splice(idx, 1);
                if (typeof throttledSaveData === 'function') throttledSaveData();
                renderComboCardList();
                showNotification('组字卡已删除', 'success');
            }
        });
    });
}

function openComboEditor(index) {
    const isEdit = index >= 0 && index < window.comboCards.length;
    const combo = isEdit ? window.comboCards[index] : { name: '', items: [], separator: ' ' };
    ...
    overlay.innerHTML = `
        <div style="background:var(--secondary-bg);border-radius:20px;padding:24px;width:90%;max-width:420px;max-height:80vh;overflow-y:auto;box-shadow:0 20px 50px rgba(0,0,0,0.3);">
            <h3 style="margin-bottom:15px;color:var(--text-primary);">${isEdit ? '编辑组字卡' : '新建组字卡'}</h3>
            <div>
                <label style="font-size:12px;color:var(--text-secondary);">名称</label>
                <input id="combo-name" class="modal-input" value="${escapeHtml(combo.name)}" placeholder="例如：早安问候">
            </div>
            <div>
                <label style="font-size:12px;color:var(--text-secondary);">分隔符</label>
                <input id="combo-sep" class="modal-input" value="${escapeHtml(combo.separator)}" placeholder="默认空格，可换行或符号">
            </div>
            <div>
                <label style="font-size:12px;color:var(--text-secondary);">字卡列表（每行一条）</label>
                <textarea id="combo-items" class="modal-textarea" style="height:180px;">${escapeHtml(combo.items.join('\n'))}</textarea>
            </div>
            <div style="text-align:right;margin-top:10px;color:var(--text-secondary);font-size:11px;">
                共 <span id="combo-count">${combo.items.length}</span> 条
            </div>
            <div style="display:flex;gap:10px;margin-top:10px;">
                <button id="combo-cancel" class="modal-btn modal-btn-secondary" style="flex:1;">取消</button>
                <button id="combo-save" class="modal-btn modal-btn-primary" style="flex:2;">保存</button>
            </div>
        </div>
    `;
    ...
    const nameInput = overlay.querySelector('#combo-name');
    const sepInput = overlay.querySelector('#combo-sep');
    const itemsTextarea = overlay.querySelector('#combo-items');
    const countSpan = overlay.querySelector('#combo-count');
    ...
    overlay.querySelector('#combo-cancel').addEventListener('click', () => overlay.remove());
    overlay.querySelector('#combo-save').addEventListener('click', () => {
        const name = nameInput.value.trim();
        if (!name) { showNotification('请填写名称', 'warning'); return; }
        const items = itemsTextarea.value.split('\n').map(l => l.trim()).filter(Boolean);
        if (items.length < 2) { showNotification('至少需要2条字卡', 'warning'); return; }
        
        const newCombo = {
            id: isEdit ? combo.id : Date.now(),
            name,
            items,
        ...
        if (isEdit) {
            window.comboCards[index] = newCombo;
        } else {
            window.comboCards.push(newCombo);
        }
        
        if (typeof throttledSaveData === 'function') throttledSaveData();
        overlay.remove();
        renderComboCardList();
        showNotification(`组字卡「${name}」已保存`, 'success');
    });
...
// 初始化：在打开字卡管理面板时，侧边栏自动加上按钮
// 监听原有的 custom-replies-function 点击
document.addEventListener('click', function(e) {
    if (e.target.closest('#custom-replies-function')) {
        setTimeout(initComboCardTab, 100);
    }
});

// 如果侧边栏已存在，立即初始化
setTimeout(initComboCardTab, 1000);
